...
export const subscriptionRequests = mysqlTable("subscriptionRequests", {
  ...
  deviceType: mysqlEnum("deviceType", ["android", "apple", "smarttv"]).notNull(),
  status: mysqlEnum("status", ["pending", "confirmed", "active", "cancelled"]).default("pending").notNull(),
  notes: text("notes"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
...
export type DeviceType = "android" | "apple" | "smarttv";